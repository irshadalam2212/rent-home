...
export interface UploadProps extends CommonProps {
    ...
    uploadLimit?: number
    buttonClass?: string
    resetFilesTrigger?: boolean; // New prop to trigger file reset
}
...
const Upload = forwardRef<HTMLDivElement, UploadProps>((props, ref) => {
        ...
        if (!draggable && !children) {
            return (
                <Button
                    ...
                    className={`bg-[#F5F5F5 border-none outline-none] ${buttonClass}`}
                >
                    Upload
                </Button>
            )
    ...
    useEffect(() => {
        if (resetFilesTrigger) {
            setFiles([]); // Clear files when resetFilesTrigger is true
            onChange?.([], []); // Notify parent that files are cleared
        }
    }, [resetFilesTrigger]);
