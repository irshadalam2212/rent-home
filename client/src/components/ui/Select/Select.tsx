...
interface MultiValueRemoveProps {
    innerProps: JSX.IntrinsicElements['div']
    data: any // This represents the option that will be removed
    onRemove: (option: any) => void // This is the custom callback you want to trigger
}
...
const CustomMultiValueRemove = ({
    ...
    //     onRemove(data); // Trigger the custom onRemove function?
    // };

    return (
        <div
            className="select-multi-value-remove"
            {...restInnerProps}
            ref={ref}
        >
...
> extends CommonProps,
    ...
    variant?: 'light' | 'default' // <-- Add this
}
                ...
                control: (state) =>
                    cn(
                        ...
                        (() => {
                            ...
                            if (variant === 'light') {
                                classes.push('bg-white dark:bg-gray-800 shadow')
                            } else {
